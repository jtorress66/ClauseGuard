...
## Change Log
- **2026-04-02 (Session 12)**: P0 FIX - Agiloft clause creation now uses SOAP EWCreate_WSClause to properly set clause_to_clause_type linked field. REST and SOAP Update cannot write swdao3link fields — only SOAP Create can. However, Agiloft KB has an automation rule that auto-assigns 552.xxx → FAR (should be GSAM) — this requires Agiloft admin fix. Also fixed multi-attachment bug: switched /contract-attachments from SOAP EWSelectAndRead (fails with >1 result) to REST search API. Clause type IDs: FAR=121, DFARS=122, GSAM=123.
