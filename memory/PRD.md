...
- [x] **NEW: Agiloft Extraction Feature** - Extract clauses with checkbox detection for Agiloft KB upload
  ...
  - **DFARS clause text fetching** - Fetches DFARS (252.x) clauses from the combined Part 252 page using anchor IDs, with caching
...
## Change Log
- **2026-04-02 (Session 12)**: P0 FIX - Agiloft REST API clause creation in Clause Library now works with correct clause type. Root cause #1: `clause_to_clause_type` (swdao3link linked field) was being sent in REST payload, causing 500 error. Fix: Removed from REST, added SOAP `EWUpdate_WSClause` to set linked type after creation (REST creates → SOAP sets type). Root cause #2: 552.xxx clauses were being tagged "GSAR"/"FAR" instead of "GSAM". Fix: Updated frontend/backend type mapping to use "GSAM" for 552.xxx clauses. Clause type IDs: FAR=121, DFARS=122, GSAM=123. All 3 GSAR test clauses created with correct GSAM type.
...
- **2026-04-02 (Session 11)**: 
  ...
  - Feature: Added "Preview & Link" step in Agiloft Integration UI — users now see a confirmation preview (target contract, clause list with FAR/DFARS badges) before bulk linking. Includes "Back to Selection" and "Confirm & Link" buttons.
  - All tests passed (3/3 backend, frontend verified).
