...
import { 
  ...
  MessageSquare, Plus, Trash2, ExternalLink, Copy, Check, LayoutDashboard, RefreshCw
} from "lucide-react";
...
export default function ClauseDetail() {
  ...
  const fetchClause = async (force = false) => {
    try {
      ...
      // First try to get formatted clause with DITA
      const url = force 
        ? `${API}/clauses/fetch-live/${clauseId}?force=true&format=dita`
        : `${API}/clauses/formatted/${clauseId}`;
      ...
      if (formattedResponse.ok) {
        const data = await formattedResponse.json();
        setClause(data);
        if (force) toast.success("Clause refreshed from acquisition.gov");
      } else {
        // Fallback to regular clause endpoint
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl border border-slate-200 p-8">
              ...
              <div>
                ...
                <div className="max-w-none clause-text">
                  {clause.formatted_text ? (
                    <div 
                      className="text-slate-700 leading-relaxed"
                      dangerouslySetInnerHTML={{ __html: clause.formatted_text }}
                      style={{
